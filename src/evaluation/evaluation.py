...
from .scripts.evaluate_diversity import get_perspectives, alpha_ndcg, calculateNormalizedDiscountedKLDivergence
from .scripts.evaluate_relevance import ndcg, precision_at_k
from .scripts.utils import read_gold_data, retrieve_argumentIDs_duplicate_texts

######################################################################
### COPIED FROM scripts.evaluation.py TO AVOID WEIRD IMPORT ERRORS ###
...
def evaluate_accuracy_sample(predictions, sample, attributes):
    '''Calculates the accuracy of the predictions for a given sample.
    ...
    Returns:
        accuracies: dictionary with the accuracy of the predictions for 
                    each attribute'''
    accuracies = {attribute:0 for attribute in attributes}
    for attribute in attributes:
        assert len(sample) == len(predictions), "The number of samples"
        " and predictions must be the same"
        pred_att = []
        for pred in predictions:
            if (pred is not None) and (f"{attribute}" in pred.keys()):
                pred_att.append(pred[f"{attribute}"][0])
            else:
                pred_att.append(None)
        accuracies[attribute] = sum([1 if a == b else 0 for a, b in \
                                      zip(pred_att, 
                                          sample[f"{attribute}"]. \
                                            to_list())]) / len(pred_att)
    return accuracies

def evaluate_predictions(predictions, data_dir, output_dir, scenario, split, diversity=True, implicit=False):
    '''Evaluate the predictions according to the organizers' evaluation script.
    Args:
    ...
    # stringIO and dumps to directly convert the list of dictionaries to a
    # jsonl object and then to a pandas dataframe
    predictions = pd.read_json(StringIO(json.dumps(predictions)))
    predictions = predictions.sort_values("query_id")

    output_dir = f"{output_dir}/{scenario}/{split}"

    os.system(f"mkdir -p {output_dir}")

    evaluate_relevance(ground_truth_df=ground_truth, predictions_df=predictions,
                       corpus=corpus, output_dir=output_dir, implicit=implicit)
    if diversity:
        evaluate_diversity(ground_truth_df=ground_truth, predictions_df=predictions,
                           corpus=corpus, output_dir=output_dir, implicit=implicit)
